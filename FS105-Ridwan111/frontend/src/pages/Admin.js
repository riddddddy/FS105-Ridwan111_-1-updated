...
const Admin = () => {
  ...
  const fetchUsers = async () => {
    try {
        const response = await fetch(`https://fs105-ridwan111-1-updated.onrender.com/api/users`);
...
const deleteUser = async (user_id) => {
  try {
    await fetch(`https://fs105-ridwan111-1-updated.onrender.com/api/users/${user_id}`, {
      method: 'DELETE',
    });
  ...
  const fetchSubscribes = async () => {
    try {
      const response = await fetch(`https://fs105-ridwan111-1-updated.onrender.com/api/subscribe`);
